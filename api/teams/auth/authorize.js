...
// Handles both legs of the Microsoft Teams OAuth flow for GHL.
//
// Leg 1 (no code): GHL initiates the flow → redirect to Microsoft.
// Leg 2 (code present): Microsoft redirects back → exchange code for tokens,
//   save to Supabase, create initial Teams meeting, return success page.
...
export default async function handler(req, res) {
  ...
  // ── Leg 1: no code → redirect to Microsoft ──────────────────────────────
  if (!code) {
  ...
  // ── Leg 2: code present → full OAuth callback handling ──────────────────

  // 1. Decode state → locationId
  let locationId;
  try {
    const decoded = Buffer.from(state, "base64").toString("utf8");
    ({ id: locationId } = JSON.parse(decoded));
  } catch (err) {
